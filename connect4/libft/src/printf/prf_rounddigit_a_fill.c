...
{
	if (pos < 0 || my_strchr(CHRS_SIGN, *(str + pos)))
	{
		my_memmove(str + pos + 2, str + pos + 1, len - pos - 1);
		*(str + pos + 1) = '1';
		return (1);
	}
	else
...
static int	prf_rounddigit_a_down(char *str, ssize_t pos, ssize_t len)
{
	if (pos < 0 || my_strchr(CHRS_SIGN, *(str + pos)))
	{
		my_memmove(str + pos + 1, str + pos + 2, len - pos - 1);
		return (-1);
	}
	else
	{
...
static char	*prf_rounddigit_a_expdown(char *str, t_flen *len)
{
	...
	my_strlcpy(str + len->sig, str + len->org - len->exp, len->exp + 1);
	return (str);
}
...
char	*prf_rounddigit_a_fill(char *str, t_conv *conv, t_flen *len)
{
	...
	if (str_new == NULL)
		return (str);
	my_strlcpy(str_new, str, len->pt);
	my_memset(str_new + len->pt - 1, CHR_ZERO, len2->pad);
	my_strlcpy(str_new + len->pt - 1 + len2->pad, str + len->pt - 1, \
		len->sig - len->pt + 2);
	my_memset(str_new + len->sig, CHR_ZERO, len2->fill);
	my_strlcpy(str_new + len2->new - len->exp, str + len->org - len->exp, \
		len->exp + 1);
	if ('8' <= num_lo && my_isxdigit(num_lo) \
