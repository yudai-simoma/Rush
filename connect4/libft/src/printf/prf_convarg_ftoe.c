...
static char	*prf_convarg_ftoe_shift(char *str, t_fpos *pos)
{
	if (pos->hi + 1 < pos->pt)
	{
		my_memmove(str + pos->hi + 2, str + pos->hi + 1, \
			pos->pt - pos->hi - 1);
	...
	else if (pos->pt < pos->hi)
	{
		*(str + pos->pt - 1) = *(str + pos->hi);
		my_memmove(str + pos->pt + 1, str + pos->hi + 1, \
			pos->end - pos->hi - 1);
...
char	*prf_convarg_ftoe(char *str)
{
	...
	*(str + pos->end + pos_e++) = CHR_EXP;
	...
	my_strlcpy(str + pos->end + pos_e, s_exp, my_strlen(s_exp) + 1);
	free(s_exp);
	free (pos);
	return (str);
}
