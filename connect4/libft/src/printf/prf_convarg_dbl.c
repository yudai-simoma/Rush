...
static char	*prf_convarg_f(long double nb, t_conv *conv)
{
	...
	if (prf_hasflag(conv->type, TYPE_E))
	...
	if (nb >= 0)
	{
		if (prf_hasflag(conv->flag, FLAG_PLUS))
			str = prf_strjoin(STR_PLUS, str, 2);
		else if (prf_hasflag(conv->flag, FLAG_SPACE))
...
static char	*prf_convarg_g(long double nb, t_conv *conv)
{
	...
	else if (my_strlen(str_e) <= my_strlen(str_f))
	{
		free(str_f);
		return (prf_convarg_g_ret(str_e, conv, precision_e));
	}
...
char	*prf_convarg_dbl(long double nb, t_conv *conv)
{
	...
	else if (prf_hasflag(conv->type, TYPE_A))
	{
	...
	else if (prf_hasflag(conv->type, TFLAG_G))
