...
static char	*prf_convarg_x(unsigned long long nb_u, t_conv *conv)
{
	...
	if (nb_u == 0 && prf_hasflag(conv->flag, FLAG_PRECISION))
		str = prf_chrtostr('\0');
	...
	if (nb_u != 0 && prf_hasflag(conv->flag, FLAG_ALT))
		str = prf_strjoin(STR_PRE_X, str, 2);
	if (prf_hasflag(conv->type, TFLAG_CAP))
		prf_strtoupper(str);
	return (str);
}
...
char	*prf_convarg_ui(unsigned long long nb_u, t_conv *conv)
{
	...
	if (prf_hasflag(conv->type, TFLAG_DEC))
		str = prf_convarg_u(nb_u, conv);
	else if (prf_hasflag(conv->type, TFLAG_OCT))
		str = prf_convarg_o(nb_u, conv);
	else if (prf_hasflag(conv->type, TFLAG_HEX))
