...
static void	prf_setconv_precision(const char **str, va_list *ap, t_conv **conv)
{
	...
	{
		if (my_strchr(CHRS_SIGN, **str) == NULL)
			precision = my_atoi(*str);
		else
			precision = 0;
		while (my_isdigit(*(*str + 1)))
			(*str)++;
	}
	if (precision < 0)
	{
		if (!prf_hasflag((*conv)->type, TFLAG_C))
...
void	prf_setconv_wandp(const char *str, va_list *ap, t_conv **conv)
{
	...
	while (str < str_end)
	{
		...
		else if (my_isdigit(*str) || *str == CHR_REF)
			prf_setconv_width(&str, &*ap, &(*conv));
		str++;
	}
	if (!prf_hasflag((*conv)->flag, FLAG_PRECISION))
	{
		if (prf_hasflag((*conv)->type, TFLAG_DBL))
