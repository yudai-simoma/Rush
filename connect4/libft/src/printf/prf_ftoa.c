...
static char	*prf_ftoa_int(long double nb)
{
	...
	while (nb >= 1)
	{
		...
		str_tmp = my_lltostr(nb - (long long)nb_tmp * NUM_LLMAX10);
...
static char	*prf_ftoa_dec(char *str, long double nb, ssize_t pos, ssize_t len)
{
	...
	while (pos < len)
	{
		...
		str_tmp = my_lltostr(nb);
		len_tmp = my_strlen(str_tmp);
		my_memset(str + pos, '0', EXP_LLMAX - len_tmp);
		my_strlcpy(str + pos + EXP_LLMAX - len_tmp, str_tmp, len_tmp + 1);
		free(str_tmp);
		pos += EXP_LLMAX;
	}
	return (str);
